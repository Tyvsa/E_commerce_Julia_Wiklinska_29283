...
export class MainService {
  ...
  {const s = new Promise((resolve, reject) => {const xhttp = new XMLHttpRequest();
    ...
    {if (this.readyState === 4 && this.status === 200) 
      ...
      else {reject('Failed');}}};});s.then((onmessage: any) => 
      {console.log('Pomyślnie dodano nowy produkt!');}).catch((onmessage) => 
...
removeProduct(id) {
  ...
  const SQL = ('object=' + encodeURIComponent(JSON.stringify(request)));
  console.log(this.apiPath + SQL);xhttp.open('GET', this.apiPath + SQL, true);xhttp.send();
  xhttp.onreadystatechange = function () {if (this.readyState === 4 && this.status === 200) 
    {const resultObject = JSON.parse(xhttp.responseText);if (resultObject !== null) 
    {resolve(resultObject);} else {reject('Failed');}}};});
    s.then((onmessage: any) => {console.log('Pomyślnie usunięto produkt!');}).catch((onmessage) => {console.log('Coś poszło nie tak podczas usuwania produktu!');});}
